...
const String word =
    ...
    "(?:[$_upperCase][$_diaCriticalMark]*)?(?:[$_lowerCase][$_diaCriticalMark]*)+|(?:[$_upperCase][$_diaCriticalMark]*)+(?![$_lowerCase])|[$_digit]+|[$_dingBatBlock]|[^$_nonChar$generalPunctuation$_whiteSpace]+";

String _camelCaseToWords(
  String subject, [
  ...
  final List<String?> words =
      pattern.allMatches(subject).map((RegExpMatch m) => m.group(0)).toList();
...
const String _defaultPattern = word;
...
class EnumToString {
  ...
  static String convertToString(dynamic enumItem, {bool camelCase = false}) {
    ...
    return !camelCase ? _tmp : _camelCaseToWords(_tmp);
  }
  ...
  static List<String> toList<T>(List<T> enumValues, {bool camelCase = false}) {
    ...
    _enumList.forEach((String p0) => output.add(p0));
  ...
  static List<T?> fromList<T>(List<T> enumValues, List<dynamic> valueList) {
    return List<T?>.from(
      valueList.map<T?>((dynamic item) => fromString(enumValues, item)),
    );
  }
}
